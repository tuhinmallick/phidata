    ...
    def volume_definition_up_to_date(self, volume_definition: Dict[str, Any]) -> bool:
        if self.name is not None:
            if volume_definition.get("name") != self.name:
                logger.debug(f'{self.name} != {volume_definition.get("name")}')
                return False
        if self.host is not None:
            if volume_definition.get("host") != self.host:
                logger.debug(f'{self.host} != {volume_definition.get("host")}')
                return False
        if self.docker_volume_configuration is not None:
            if volume_definition.get("dockerVolumeConfiguration") != self.docker_volume_configuration:
                logger.debug(
                    f'{self.docker_volume_configuration} != {volume_definition.get("dockerVolumeConfiguration")}'
                )
                return False
        if self.efs_volume_configuration is not None:
            if volume_definition.get("efsVolumeConfiguration") != self.efs_volume_configuration:
                logger.debug(
                    f'{self.efs_volume_configuration} != {volume_definition.get("efsVolumeConfiguration")}'
                )
                return False
        if self.fsx_windows_file_server_volume_configuration is not None:
            if (
                ...
                logger.debug(
                    f'{self.fsx_windows_file_server_volume_configuration} != {volume_definition.get("fsxWindowsFileServerVolumeConfiguration")}'
                )
                return False
